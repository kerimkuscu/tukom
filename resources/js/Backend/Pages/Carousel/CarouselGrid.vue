<template>
  <div>
    ...
    <DataTable
      ...
      @sort="onSort($event)"
    >
      <Column ref="name" field="name" :header="$t('carousel.columns.name')" />
      <Column :header="$t('carousel.columns.image')">
        <template #body="slotProps">
          <img :src="'/images/' + slotProps.data.image" :alt="slotProps.data.image" class="carousels-image" style="height: 30px">
        </template>
      </Column>
...
export default {
    ...
    methods: {
        async fetch(){
            ...
            const response = await this.$http.get('/api/carousels' + queryString)
        ...
        create(){
            this.$router.push({ name: 'carousels.create' });
        },
        ...
        remove(id){
            this.$confirm.require({
                ...
                accept: async () => {
                    const response = await this.$http.delete('/api/carousels/' + id)
                    if(response.data.status){
                        this.$toast.add({ severity:'error', detail:this.$i18n.t('carousel.messages.not_deleted'), life: 2000 });
                    }
                    else{
                        this.$toast.add({ severity:'success', detail: this.$i18n.t('carousel.messages.deleted'), life: 2000 });
                    }
