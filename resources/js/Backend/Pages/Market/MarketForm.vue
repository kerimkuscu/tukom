  ...
  <form @submit.prevent="submit">
    <div class="pb-5" style="margin-bottom: 20px">
      <h4 class="float-start card-title">
        {{ $t('market.title') }} / {{ createOrEditPage }}
      </h4>
    ...
    <div class="row">
      <div class="col-8 mx-auto">
        <div class="form-group row mb-2">
          <label class="col-form-label col-md-3 required">{{ $t('market.form.image') }}</label>
        ...
        <div class="form-group row mb-2">
          <label class="col-form-label col-md-3 required">{{ $t('market.form.name') }}</label>
          <div class="col-md-9 grid p-fluid">
            <InputText
              id="name"
              v-model="form.name"
...
export default {
    ...
    methods : {
        async fetch(){
            const response = await this.$http.get('/api/markets/' + this.$route.params.id);
        ...
        async store() {
            try {
                await this.form.post('/api/markets');
                this.$toast.add({ severity:'success', detail: this.$i18n.t('market.messages.created'), life: 2000 });
                setTimeout(() => {
                    this.$router.push({ name: 'markets.grid' });
                }, 500);
            } catch (error) {
                if(error.response.status !== 422) {
                    this.$toast.add({ severity:'error', detail: this.$i18n.t('market.messages.not_created'), life: 2000 });
                }
            }
        },
        ...
        async update() {
            try {
                ...
                if(error.response.status !== 422) {
                    this.$toast.add({ severity:'error', detail: this.$i18n.t('market.messages.not_updated'), life: 2000 });
                }
            }
        },
