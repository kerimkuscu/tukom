<template>
  <div>
    ...
    <DataTable
      ...
      @sort="onSort($event)"
    >
      <Column ref="name" field="name" :header="$t('firm.columns.name')" />
      <Column :header="$t('firm.columns.image')">
        <template #body="slotProps">
          <img :src="'/images/' + slotProps.data.image" :alt="slotProps.data.image" class="firms-image" style="height: 30px">
        </template>
      </Column>
...
export default {
    ...
    methods: {
        async fetch(){
            ...
            const response = await this.$http.get('/api/firms' + queryString)
        ...
        create(){
            this.$router.push({ name: 'firms.create' });
        },
        ...
        remove(id){
            this.$confirm.require({
                ...
                accept: async () => {
                    const response = await this.$http.delete('/api/firms/' + id)
                    if(response.data.status){
                        this.$toast.add({ severity:'error', detail:this.$i18n.t('firm.messages.not_deleted'), life: 2000 });
                    }
                    else{
                        this.$toast.add({ severity:'success', detail: this.$i18n.t('firm.messages.deleted'), life: 2000 });
                    }
